...
public class ClientEvents {
    ...
    private static void placePing(Minecraft mc) {
        ...
        Vec3 reachEnd = eye.add(look.scale(Spotter.MAX_PING_DISTANCE));
        ...
        double maxDistSq = blockHit.getType() == HitResult.Type.MISS
                ? Spotter.MAX_PING_DISTANCE * Spotter.MAX_PING_DISTANCE
                : blockHit.getLocation().distanceToSqr(eye);

        AABB reachBox = player.getBoundingBox().expandTowards(look.scale(Spotter.MAX_PING_DISTANCE)).inflate(1.0);
    ...
    private static void dispatch(Minecraft mc, PlacePingPayload payload) {
        ...
        try {
            ...
            LOGGER.warn("[Spotter] Server doesn't accept the ping channel — switching to client-only mode for this connection.", t);
            clientOnlyMode = true;
